...
class EsportsExperienceController {
  ...
  async show_exp({auth, request, response}){
    try{
      const myesportsExperience = await EsportsExperience.query().where({id: request.params.esportsExp_id}).fetch()
      return {
        myesportsExperience
      }
    }
  ...
  async update({auth, request, response}){
    try{
      const updateesports_Exp = await EsportsExperience.query().where({id: request.params.id}).update({role_title: request.input('role_title'), game_name: request.input('game_name'), team_name: request.input('team_name'), duration: request.input('duration'), achievements: request.input('achievements'), skills: request.input('skills')})
  ...
  async destroy({auth, request, response}){
    if(auth.user){
      try{
        const delete_esport_exp = await Database.table('esports_experiences').where({
  ...
  async show({auth, request, response}){
    try{
      const esportsExperience = await EsportsExperience.query().where({user_id: request.params.id}).fetch()
      return {
        esportsExperience
      }
    }
  ...
  async esportsSearchResults({auth, request, response}){
    try{
      ...
      const latestGameExperiences = await Database.schema.raw("select users.id AS user_id, users.alias, users.profile_img, esports_experiences.id, esports_experiences.game_name, esports_experiences.role_title, esports_experiences.team_name, esports_experiences.duration, esports_experiences.skills from esports_experiences Inner Join users ON esports_experiences.user_id = users.id" + mySQL)
