...
class CommonSaveController {
  async register({ view , session }){
      ...
      return view.render('auth/socialRegister',{alias: session.get('alias'),email: session.get('email')});
  }
  ...
  async saveuser({ ally, auth, request, session, response, view  }){
    ...
    if (validation.fails()) {
      ...
      const mailOptions = {
          ...
          html: '<h1>Hello,'+request.input("firstName")+' '+request.input("lastName")+' .</h1><p>Welcome to the My Game, here is your getting started guide</p>'
      };
