...
class DiscordLoginController {
  async redirect ({ ally, response }) {
    ...
    return response.redirect('https://discordapp.com/api/oauth2/authorize?client_id=588326792289320962&redirect_uri=https://myg.gg/authenticated%2Fdiscord&response_type=code&scope=email%20identify')
  ...
  async callback ({ auth, response ,request , session  }) {
    ...
    if(code){
      ...
      const res = await fetch(`https://discordapp.com/api/oauth2/token?grant_type=authorization_code&code=${code}&redirect_uri=https://myg.gg/authenticated/discord`,
        {
          method: 'POST',
          headers: {
