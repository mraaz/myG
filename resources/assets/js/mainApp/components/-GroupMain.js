...
export default class GroupMain extends Component {
  ...
  togglePostTypeTab = (label) => {
    ...
    if (label == 'suggested-community') {
      open_compose_textTab = false
    }
    if (label == 'your-community') {
      open_compose_textTab = false
    }
  ...
  render() {
    ...
    return (
      <section id='group-page' class='postCompose__container '>
        <div className='compose__type__section'>
          <div
            className={`share__thought ${open_compose_textTab ? 'active' : ''}`}
            onClick={(e) => this.togglePostTypeTab('your-community')}>{`Your Communities`}</div>
          <div className='devider'></div>
          <div
            className={`add__post__image ${open_compose_textTab ? '' : 'active'}`}
            onClick={(e) => this.togglePostTypeTab('suggested-community')}>{`Suggested Communities`}</div>
        </div>
        {open_compose_textTab && (
          <div className='communities_cards'>
            <h1>Your Communities Cards</h1>
          </div>
        )}
        {open_compose_textTab && (
          <div className='suggested__communities'>
            <h1>Your Communities Cards</h1>
          </div>
        )}
        {/* <GroupOpenModal
