        ...
        <div className="padding-container">
          <div className="grey-container">
            <div className="author-info">
              {this.state.show_profile_img && <a href={`/profile/${post.user_id}`} className="user-img" style={{
              ...
              <div className="info">
                <a href={`/profile/${post.user_id}`}>{`${post.first_name} ${post.last_name}`}</a> shared a <a href={`/profile/${post.user_id}`}>{(post.type == 'text') ? 'story' : 'image'}</a>
              </div>
              { this.state.show_post_options && <div className="post-options">
              ...
              <div className={`post-dropdown ${this.state.dropdown ? 'active' : ''}`}>
                <nav>
                  ...
                  <div className="delete" onClick={() => { if (window.confirm('Are you sure you wish to delete this post?')) this.delete_exp() } }>Delete</div>&nbsp;
                </nav>
              </div>
            </div>
            <div className="media">
              {!this.state.edit_post && <div className="update-info">
                <p>
                  {this.state.content}
                </p>
              </div>}
              ...
              </div>}
              {
                media_urls.map(function(data, index){
                  if(post.type == 'photo'){
                    return (
                      <img className="post-photo" src={data.src} key={data.key}></img>
                    )
                  }
              ...
              <div className="update-stats">
                ...
                {this.state.show_like && <div className="other-users">
                ...
                {this.state.like && <div className="like-btn" onClick={() => this.click_unlike_btn(post.id)} >
                  <i className="fas fa-thumbs-up"/>&nbsp;Like
                </div>}
                {!this.state.like && <div className="like-btn" onClick={() => this.click_like_btn(post.id)} >
                  <i className="far fa-thumbs-up"/>&nbsp;Like
                </div>}
                {this.state.zero_comments && <div className="comments-stats" onClick={this.onChange}> {this.state.comment_total > 1 ? `${this.state.comment_total} comments` : `${this.state.comment_total} comment`} </div>}
                {!this.state.zero_comments && <div className="comments-stats" onClick={this.focusTextInput}> No comments</div>}
              </div>
              <div className="compose-comment">
                <textarea
                  name="name"
