...
export default class IndividualComment extends Component {
  ...
  render() {
    ...
    if(this.state.comment_deleted != true) {
      return (<div className="individual-comment-container">
        <div className="author-info">
          {this.state.show_profile_img && <a href={`/profile/${comment.user_id}`} className="user-img" style={{
            backgroundImage: `url('${comment.profile_img}')`}}/>}
          {!this.state.show_profile_img && <a href={`/profile/${comment.user_id}`} className="user-img" style={{
            backgroundImage: `url('https://image.flaticon.com/icons/svg/149/149071.svg')`
          }}/>}
          <div className="comment-info">
            <a href={`/profile/${comment.user_id}`}>{`${comment.first_name} ${comment.last_name}`}</a>
          </div>
          { this.state.show_comment_options && <div className="comment-options">
            <i className="fas fa-ellipsis-h" onClick={this.clickedDropdown}></i>
          </div> }
          <div className={`dropdown ${this.state.dropdown ? 'active' : ''}`}>
            <nav>
              <div className="edit" onClick={this.clickedEdit}>Edit &nbsp;</div>
              <div className="delete" onClick={() => { if (window.confirm('Are you sure you wish to delete this comment?')) this.delete_exp() } }>Delete</div>&nbsp;
            </nav>
          </div>
        </div>
        ...
        <div className="comment-panel">
          {this.state.like && <div className="comment-panel-liked" onClick={() => this.click_unlike_btn(comment.id)}>
            Like
          </div>}
          {!this.state.like && <div className="comment-panel-like" onClick={() => this.click_like_btn(comment.id)}>
            Like
          </div>}
          <div className="comment-panel-reply" onClick={this.toggleReply}>Reply
          </div>
          {(this.state.show_like || this.state.show_reply) && <div className="divider">|</div>}
          {this.state.show_like && <div className="no-likes">{this.state.total} {this.state.total > 1 ? "Likes" : "Like"} </div>}
          {(this.state.show_reply && this.state.show_like) && <div className="colon">:</div>}
          {this.state.show_reply && <div className="no-reply"> {this.state.reply_total} {this.state.reply_total > 1 ? "Replies" : "Reply"}</div>}
          <div className="comment-time">
            <i className="fas fa-circle"></i> {this.state.comment_time}
          </div>
        </div>
        {this.state.show_more_replies && this.showReplies()}
        {this.state.show_add_reply && <div className="add-reply">
          <input type="text" id="reply_name_box" className="reply-name-box" placeholder="Add a reply..." onKeyUp = {this.detectKey} ref={this.setTextInputRef} onChange={this.handleChange} value={this.state.value}/>
        </div>}
        {this.state.show_edit_comment && <div className="add-reply">
          <input type="text" id="reply_name_box" className="reply-name-box" onKeyUp = {this.detectKey2} ref={this.setTextInputRef} onChange={this.handleChange2} value={this.state.value2}/>
        </div>}
      </div>)
    } else{
      return (
