...
export default class IndividualInvitation extends Component {
  ...
  clickedAccept = () => {
    if(this.state.group_approvals){
      try{
        const set_group_approval = axios.get(`/api/usergroup/set_group_approval/${this.props.invitation.group_id}/${this.props.invitation.id}`)
      } catch(error){
        console.log(error)
      }
  ...
  render() {
    ...
    return (
      <div className="invitation-info">
        {show_profile_img && <a href={`/profile/${invitation.user_id}`} className="user-img" style={{
