...
export class GamerSuggestions extends React.Component {
  ...
  renderGamerSuggestion = (profile, index) => {
    ...
    return (
      <div
        ...
        key={`${profile.alias}_${index}`}
        onClick={() =>
          this.props.onboarding ? this.sendFriendRequest(profile.alias, profile.profileId) : window.router.push(`/profile/${profile.alias}`)
        }
      ...
      >
        ...
        <div
          className='image absolute-top'
          style={{
            backgroundImage: `url(${profile.background}), url(https://myg.gg/platform_images/Profile/Silver-Stamping-Logo-MockUp.jpg)`
          }}
        />
        <div
          className='icon'
          style={{ backgroundImage: `url(${profile.image}), url(https://myG.gg/default_user/new-user-profile-picture.png)` }}
        />
        ...
        {games
          ...
          .slice(0, 3)
          .map((game, index) => (
            <div className='field center' key={`game_suggestion_${game.name}_${index}`}>
    ...
    return (
      <div className='hover-bar suggestion-bar'>
        {!this.props.onboarding && (
          <div
            className='small-button suggestion-button clickable'
            onClick={(event) => {
              event.stopPropagation()
              window.router.push(`/profile/${gamer.alias}`)
            }}
          >
            Profile
          </div>
        )}
        {this.props.profile && !isFriend && !hasSentRequest && (
          <div
            className='small-button suggestion-button clickable'
            onClick={(event) => {
              event.stopPropagation()
              this.sendFriendRequest(gamer.alias, gamer.profileId)
            }}
          >
            Connect
          </div>
        ...
        {this.props.profile && !isFriend && hasSentRequest && (
          <div
            ...
            onClick={(event) => {
              event.stopPropagation()
              this.cancelFriendRequest(gamer.alias, gamer.profileId)
            }}
          >
            Requested
          </div>
        )}
        {!this.props.onboarding && this.props.profile && (
          <div
            className='small-button suggestion-button clickable'
            onClick={(event) => {
              event.stopPropagation()
        ...
        {!this.props.onboarding && this.props.profile && !isFollower && (
          <div
            className='small-button suggestion-button clickable'
            onClick={(event) => {
              event.stopPropagation()
              this.follow(gamer.alias, gamer.profileId)
            }}
          >
            Follow
          </div>
        ...
        {!this.props.onboarding && this.props.profile && isFollower && (
          <div
            ...
            onClick={(event) => {
              event.stopPropagation()
              this.unfollow(gamer.alias, gamer.profileId)
            }}
          >
            Unfollow
          </div>
  ...
  changePage = (direction) => {
    ...
    if (direction === 'right')
      return this.setState({ page: page + 1 > this.props.gamerSuggestions.length - this.getGamerSuggestionsPerPage() ? page : page + 1 })
  }
  getGamerSuggestionsPerPage = () => {
    const selfSize = 1
    const othersSize = 1
  ...
  renderPageButtons = () => {
    const fitsAllInScreen = this.props.gamerSuggestions.length <= this.getGamerSuggestionsPerPage()
    const contentToTheLeft = !fitsAllInScreen && this.state.page > 0
    const contentToTheRight = !fitsAllInScreen && this.state.page < this.props.gamerSuggestions.length - this.getGamerSuggestionsPerPage()
  ...
  renderInviteModal = () => {
    ...
    return (
      <div id='find-gamers'>
        <InviteModal gamer={this.state.inviting} onClose={() => this.setState({ inviting: null })} />
      </div>
    )
  }
    ...
    return (
      <div id='profile-game-experiences'>
        ...
        <div className='scroll'>
          ...
          {suggestions
            .slice(this.state.page, this.state.page + this.getGamerSuggestionsPerPage())
            .map((profile, index) => this.renderGamerSuggestion(profile, index))}
