    ...
    if(this.state.reply_deleted != true) {
      return(
        <div className="comment-replies">
          {this.state.show_profile_img && <a href={`/profile/${reply.user_id}`} className="user-img-reply" style={{
            backgroundImage: `url('${reply.profile_img}')`}}/>}
          {!this.state.show_profile_img && <a href={`/profile/${reply.user_id}`} className="user-img-reply" style={{
            backgroundImage: `url('https://image.flaticon.com/icons/svg/149/149071.svg')`
          }}/>}
          <div className="reply-author-info" >{reply.first_name} {reply.last_name}
            { this.state.show_reply_options && <div className="reply-options">
          ...
          <div className={`reply-dropdown ${this.state.dropdown ? 'active' : ''}`}>
            <nav>
              ...
              <div className="delete" onClick={() => { if (window.confirm('Are you sure you wish to delete this reply?')) this.delete_exp() } }>Delete</div>&nbsp;
            </nav>
          </div>
          <div className="reply-comment-content">{this.state.content}
          </div>
          <div className="replies-panel">
            <div className="comment-panel">
              {this.state.reply_like && <div className="comment-panel-liked" onClick={() => this.click_reply_unlike_btn(reply.id)}>
                Like
              </div>}
              {!this.state.reply_like && <div className="comment-panel-like" onClick={() => this.click_reply_like_btn(reply.id)}>
                Like
              </div>}
              {(this.state.show_reply_like || this.state.show_reply) && <div className="divider">|</div>}
              {this.state.show_reply_like && <div className="no-likes">{this.state.reply_like_total} {this.state.reply_like_total > 1 ? "Likes" : "Like"} </div>}
              <div className="comment-time">
                <i className="fas fa-circle"></i> {this.state.reply_time}
              </div>
            </div>
          </div>
          {this.state.show_edit_reply && <div className="add-reply">
            <input type="text" id="reply_name_box" className="reply-name-box" onKeyUp = {this.detectKey} ref={this.setTextInputRef} onChange={this.handleChange} value={this.state.value}/>
          </div>}
        </div>
      )
    } else{
