    ...
    return (
      <div>
        <div className='messenger-new-group-button clickable' onClick={() => this.setState({ showingGroupCreation: true })}>
          <div className='messenger-new-group-button-icon' style={{ backgroundImage: `url(/assets/svg/ic_chat_group_create.svg)` }} />
          Create Group
        </div>
          ...
          <div className='messenger-empty-message-container'>
            ...
            <p className='messenger-empty-message'>
              {this.props.contacts.length
                ...
                : 'Try using the Add game feature or increase your network '}
            </p>
          </div>
        )}
        {this.state.showingGroupCreation && (
          <GroupCreation onCreate={this.createGroup} onCancel={() => this.setState({ showingGroupCreation: false })} />
        )}
      </div>
    )
  }
