...
export default class ChatMessage extends React.Component {
  ...
  renderReactions = () => {
    ...
    const hasReaction = (reactionId) =>
      this.props.message.reactions.find((reaction) => reactionId === reaction.reactionId && this.props.userId === reaction.senderId)
        ...
        <div className={`chat-component-message-reactions chat-component-message-reactions-${origin}`}>
          {reactions.map((reactionId) => {
            ...
            return (
              <div
                key={reactionId}
                className={`chat-component-message-reaction-container${hasReaction(reactionId) ? '-sent' : ''} clickable`}
                ...
                {this.state.reaction === reactionId && (
                  <div className='chat-component-message-reaction-senders-container'>
                    <div className='chat-component-message-reaction-senders'>
                      {reaction.map(({ senderName }) => (
                        <div className='chat-component-message-reaction-sender' key={senderName}>
                          {senderName}
                        </div>
                      ))}
                    </div>
            ...
            <div className={`chat-component-message-content`}>
              {message.deleted
                ? message.isAttachment
                  ? `The file ${this.getAttachmentName(message.content)} was automatically deleted.`
                  : origin === 'sent'
          ...
          <div className='chat-component-message-content-info'>
            <div className='chat-component-message-side-container'>
              <div
                ...
                className={`chat-component-message-options-button ${
              ...
              <div className='chat-component-message-options-menu-container'>
                <div className='chat-component-message-options-menu-inner-container'>{this.renderReactionOptions()}</div>
              </div>
              <div className='chat-component-message-options-menu-container'>
                <div className='chat-component-message-options-menu-inner-container'>{this.renderOptions()}</div>
              </div>
            </div>
            <div className='chat-component-message-side-container'>
              {!!message.edited && !message.deleted && <p className='chat-component-message-edited'>edited</p>}
              <p className='chat-component-message-date'>{formatAMPM(new Date(message.createdAt))}</p>
            </div>
          </div>
        </div>
