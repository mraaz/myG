...
class CommendationController {
  ...
  async showUser({auth, request, response}){
    try{
      const getCommend = await Database.from('commendations').where({game_experiences_id: request.params.id, user_id: auth.user.id}).count('* as no_of_commends')
  ...
  async show({auth, request, response}){
    try{
      const getAllCommend = await Database.from('commendations').where({game_experiences_id: request.params.id}).count('* as no_of_commends')
