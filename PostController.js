...
class PostController {
  ...
  async show({auth, request, response}){
    try{
      ...
      const subquery = Database.select('friend_id').from('friends').where({user_id: auth.user.id})
      const myPosts = await Database.from('posts').innerJoin('users', 'users.id', 'posts.user_id').whereIn('posts.user_id', subquery).select('*', 'posts.id', 'posts.updated_at').orderBy('posts.created_at', 'desc').paginate(request.params.paginateNo, 10)
  ...
  async showpost({auth, request, response}){
    try{
      const myPost = await Database.from('posts').innerJoin('users', 'users.id', 'posts.user_id').where('posts.id', '=', request.params.id).select('*', 'posts.id', 'posts.created_at','posts.updated_at')
  ...
  async showmyposts({auth, request, response}){
    try{
      ...
      const myPosts = await Database.from('posts').innerJoin('users', 'users.id', 'posts.user_id').where({user_id: auth.user.id}).select('*', 'posts.id', 'posts.updated_at').orderBy('posts.created_at', 'desc').paginate(request.params.paginateNo, 10)
  ...
  async get_group_posts({auth, request, response}){
    try{
      ...
      const groupPosts = await Database.from('posts').innerJoin('users', 'users.id', 'posts.user_id').where({group_id: request.params.id}).select('*', 'posts.id', 'posts.updated_at').orderBy('posts.created_at', 'desc').paginate(request.params.paginateNo, 10)
  ...
  async posts_count({auth, request, response}){
    try{
      ...
      const no_of_my_posts = await Database.from('posts').where({id: request.params.id, user_id: auth.user.id}).count('* as no_of_my_posts')
  ...
  async destroy({auth, request, response}){
    if(auth.user){
      try{
        const delete_post = await Database.table('posts').where({
  ...
  async update({auth, request, response}){
    try{
      const updatePost = await Post.query().where({id: request.params.id}).update({content: request.input('content')})
