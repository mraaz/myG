...
class EsportsBioController {
  ...
  async update({auth, request, response}){
    try{
      const updateEsports_Exp = await EsportsBio.query().where({user_id: auth.user.id}).update({status: request.input('status'), email_visibility: request.input('email_visibility'), games_of_ardour: request.input('games_of_ardour'), career_highlights: request.input('career_highlights') })
