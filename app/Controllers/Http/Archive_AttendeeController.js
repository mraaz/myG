...
class Archive_AttendeeController {
  ...
  async show_attending({ auth, request, response }) {
    try {
      ...
      const allAttendees = await Database.from('archive_attendees')
  ...
  async show_Allattending({ auth, request, response }) {
    try {
      const allAttendees = await Database.from('archive_attendees').where({
  ...
  async show_myattendance({ auth, request, response }) {
    try {
      //const allAttendees = await Database.select('*').from('archive_attendees').where({schedule_games_id: request.params.id})
      const myattendance = await Database.from('archive_attendees')
        .where({
          schedule_games_id: request.params.id,
          user_id: auth.user.id,
          type: 1,
        })
        .count('* as no_of_myAttendance')
      const myattendance_pending = await Database.from('archive_attendees')
        .where({
          schedule_games_id: request.params.id,
          user_id: auth.user.id,
          type: 3,
        })
        .count('* as no_of_myAttendance_pending')
  ...
  async remove_myattendance({ auth, request, response }) {
    if (auth.user) {
      try {
        const delete_attendance = await Database.table('archive_attendees')
          .where({
            schedule_games_id: request.params.id,
            user_id: auth.user.id,
          })
  ...
  async delete_invite({ auth, request, response }) {
    if (auth.user) {
      try {
        const delete_invite = await Database.table('archive_attendees')
          .where({
  ...
  async up_invite({ auth, request, response }) {
    try {
      const up_invite = await Archive_Attendee.query()
        .where({
          schedule_games_id: request.params.schedule_game_id,
          user_id: request.params.id,
        })
