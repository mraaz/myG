...
class AttendeeController {
  ...
  async show_attending({auth, request, response}){
    try{
      ...
      const allAttendees = await Database.from('attendees').where({schedule_games_id: request.params.id, type: 1}).count('* as no_of_allAttendees')
  ...
  async show_game_positions({auth, request, response}){
    try{
      ...
      const game_position_of_dota_2_position_ones = await Database.from('attendees').where({schedule_games_id: request.params.id, type: 1, dota_2_position_one:1}).count('* as no_of_dota_2_position_ones')
      const game_position_of_dota_2_position_twos = await Database.from('attendees').where({schedule_games_id: request.params.id, type: 1, dota_2_position_two:1}).count('* as no_of_dota_2_position_twos')
      const game_position_of_dota_2_position_threes = await Database.from('attendees').where({schedule_games_id: request.params.id, type: 1, dota_2_position_three:1}).count('* as no_of_dota_2_position_threes')
      const game_position_of_dota_2_position_fours = await Database.from('attendees').where({schedule_games_id: request.params.id, type: 1, dota_2_position_four:1}).count('* as no_of_dota_2_position_fours')
      const game_position_of_dota_2_position_fives = await Database.from('attendees').where({schedule_games_id: request.params.id, type: 1, dota_2_position_five:1}).count('* as no_of_dota_2_position_fives')
  ...
  async role_call({auth, request, response}){
    try{
      ...
      const role_call = await Database.from('attendees').innerJoin('users', 'users.id', 'attendees.user_id').select('users.id as user_id', 'users.profile_img').where({schedule_games_id: request.params.id, type: 1}).limit(6)
  ...
  async role_call_ALL({auth, request, response}){
    try{
      ...
      const role_call_ALL = await Database.from('attendees').innerJoin('users', 'users.id', 'attendees.user_id').select('users.id as user_id', 'users.profile_img', 'users.first_name', 'users.last_name').where({schedule_games_id: request.params.id, type: 1})
  ...
  async delete_invite({auth, request, response}){
    if(auth.user){
      try{
        const delete_invite = await Database.table('attendees').where({
