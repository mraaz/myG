...
def main():
  ...
  for x in range(start, stop, 1):
    ...
    if (create_celebrity and x == start):
      ...
      if(adonisJS):
        print(".raw(\"INSERT INTO users (id, alias, first_name, last_name, email, password, profile_img, status, last_seen, public_key, country, contact_info, relationship_status, profile_bg, created_at, updated_at) values (%s,'%s','%s','%s','%s','$2a$10$JTLVsD59n1jZPN3yGIn.4OUNR0EBRSiT1JrHllsyL0.pz2hh994hm','https://s3-ap-southeast-2.amazonaws.com/mygame-media/default_user/new-user-profile-picture.png','%s', '%s', 'lVxjAuGfXnA3KewnMfxuT91Wnz2nqW5XY1kYxrJWlxY41Nm2wmYcSXdsj+35gUAxFL/n/d7azGuX4uks1h+BsB5Z5d0FGSbAhhOpfKcvlrc7DvHISNEFN3jTh9DDa+ClpmBnQ7mz79fWQS2EzFhzactKpNVteL2xNy9uNK9AHKs=', '%s', 'mnraaz@gmail.com', '%s', 'https://s3-ap-southeast-2.amazonaws.com/mygame-media/default_user/universe.jpg', '2019-12-01 00:00:00', '2019-12-01 00:00:00')\")" % (x, alias[random.randint(1,(len(alias)-1))] + str(random.randint(1,88)), fake.first_name(), fake.last_name(), alias[random.randint(1,len(alias))] + str(random.randint(1,88)) + "@gmail.com" , random.choice(status), fake.date(pattern='%Y-%m-%d 00:00:00', end_datetime=None), fake.destination(), random.choice(relationship_Status)) )
      else:
    ...
    if(adonisJS):
      ...
      print(".raw(\"INSERT INTO `groups` (id, user_id, name, group_img, type, created_at, updated_at) values (%s, %s, '%s', 'https://mygame-media.s3-ap-southeast-2.amazonaws.com/stock_images/samuel-67197_1280.jpg', %s, '2019-12-01 00:00:00', '2019-12-01 00:00:00')\")" % (x, x, alias[random.randint(1,(len(alias)-1))], random.randint(1,3) ) )
    else:
      ...
      print("INSERT INTO `groups` (id, user_id, name, group_img, type, created_at, updated_at) values (%s, %s, '%s', 'https://mygame-media.s3-ap-southeast-2.amazonaws.com/stock_images/samuel-67197_1280.jpg', %s, '2019-12-01 00:00:00', '2019-12-01 00:00:00');" % (x, x, alias[random.randint(1,(len(alias)-1))], random.randint(1,3) ) )
